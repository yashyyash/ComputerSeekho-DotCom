...
const AddCourse = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.courseId) {
        await axios.put("https://localhost:7094/api/Course", payload);
        alert("Course updated successfully!");
      } else {
        await axios.post("https://localhost:7094/api/Course", payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this course?")) {
      try {
        await axios.delete(`https://localhost:7094/api/Course/${id}`);
