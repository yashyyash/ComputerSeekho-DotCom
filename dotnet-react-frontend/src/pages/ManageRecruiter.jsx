...
const ManageRecruiter = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (formData.recruiterId) {
        await axios.put("https://localhost:7094/api/Recruiter", payload);
        alert("Recruiter updated successfully!");
      } else {
        await axios.post("https://localhost:7094/api/Recruiter", payload);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Are you sure you want to delete this recruiter?")) {
      try {
        await axios.delete(`https://localhost:7094/api/Recruiter/${id}`);
